...
import os, sys, time
sys.path.append('..')
sys.path.append('../experiment_nonlinear/')
...
from physics_nonlinear import analytical_fn
...
def new_get_dataset(rng, samples=1, t_span=[0, 1], fps=1000, test_split=0.5, lookahead=1,
    ...
    # ], axis=1)
    y0 = jnp.concatenate([
        jax.random.uniform(rng, (samples, 1))*0.5,
...
data = new_get_dataset(jax.random.PRNGKey(0), t_span=[0, dataset_size], fps=fps, samples=samples, test_split=0.5)
