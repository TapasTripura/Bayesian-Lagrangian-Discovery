...
def hamiltonian_fn(coords):
    m, k1, k2, k3 = 1, 5000, 5000, 5000
    q1, q2, q3, p1, p2, p3 = np.split(coords,6)
    H = 0.5*(1/m)*(p1**2 + p2**2 + p3**2) + 0.5*k1*q1**2 + \
        0.5*k2*(q2 - q1)**2 + 0.5*k3*(q3 - q2)**2 # 3DOF hamiltonian (linear oscillator)
    return H
...
# get initial state
# if y0 is None:
#     y0 = np.random.uniform(0, 2, size=6)
# if radius is None:
#     radius = np.random.normal(0.9, 1.2) # sample a range of radii
# y0 = y0 / np.sqrt((y0**2).sum()) * radius ## set the appropriate radius

# get initial state
if y0 is None:
    y0_temp = np.random.uniform(0.1, 1.0, size=3)
    y0 = np.zeros(6)
    y0[::2] = y0_temp
