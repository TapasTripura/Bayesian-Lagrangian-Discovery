...
class MLP(torch.nn.Module):
  ...
  def __init__(self, input_dim, hidden_dim, output_dim, nonlinearity='tanh'):
    ...
    self.linear2 = torch.nn.Linear(hidden_dim, int(1.5*hidden_dim))
    self.linear3 = torch.nn.Linear(int(1.5*hidden_dim), int(2*hidden_dim))
    self.linear4 = torch.nn.Linear(int(2*hidden_dim), int(1.5*hidden_dim))
    self.linear5 = torch.nn.Linear(int(1.5*hidden_dim), hidden_dim)
    self.linear6 = torch.nn.Linear(hidden_dim, output_dim, bias=None)
  ...
  def forward(self, x, separate_fields=False):
    h = self.nonlinearity( self.linear2( self.nonlinearity( self.linear1(x) ) ) ) 
    h = self.nonlinearity( self.linear4( self.nonlinearity( self.linear3(h) ) ) )
